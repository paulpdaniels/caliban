...
This non-published project runs the pinned Federation Gateway Audit against Caliban's native, code-first gateway
composition. For each upstream suite the adapter fetches the upstream `subgraphs` descriptions, hands their authored SDL
and endpoints straight to `Gateway.compose`, and serves the resulting `GatewayInterpreter`. It neither consumes nor
produces a serialized supergraph artifact.
