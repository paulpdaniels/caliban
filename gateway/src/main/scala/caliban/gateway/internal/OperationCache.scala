...
private[gateway] final class OperationCache[K, E, V, -R] private (
  ...
  private def observe[R0 <: R, E0, A](
    ...
    if (!phases.enabled) effect
    else
      phases.cacheAccess.run(Event.CacheAccess(value))(effect)(
        Result.classifyExit
      )
}
...
private[gateway] object OperationCache {
  ...
  def make[K, E, V, R](maxWeight: Long, phases: PhaseHooks[R])(implicit
    trace: Trace
  ): UIO[OperationCache[K, E, V, R]] =
    Ref.make(State.empty[K, E, V]).map(new OperationCache(maxWeight, _, phases))
