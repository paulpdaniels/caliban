...
private[gateway] final class GatewayExecutionControl[-R] private (
  ...
  def runObservedRequest[R1 <: R, B, A](event: Event.Request, reservation: Option[Lease] = None)(
    ...
    def observe(effect: URIO[R1, A]): URIO[R1, A] = phases.request.run(event)(effect)(result)
...
private[gateway] object GatewayExecutionControl {
  def make[R](
    ...
    for {
      requests      <- AdmissionGate.make(requestLimit, AdmissionKind.Request, phases)
      subscriptions <- SubscriptionControl.make(subscriptionConfig, requests, phases)
